		...
		<div class="col-md-6">
			<form method="post">
				<div class="form-group">
					...
					<input type="text" name="name" class="form-control" id="name" value="{$template->name}" required>
				</div>

				<div class="form-group">
					<label for="image">Chemin de l'image</label>
					<input type="text" name="image" class="form-control" id="image" value="{$template->image}" required>
				</div>
				...
				<div class="form-group">
					<label for="cluster">Cluster</label>
						...
						{foreach from=$clusters item=$cluster}
							<option value="{$cluster->id}" {if $template->cluster_id === $cluster->id} selected {/if}>{$cluster->name}</option>
						{/foreach}
					</select>
				</div>
